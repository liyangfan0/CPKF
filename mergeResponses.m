function [response] = mergeResponses(response_cf_hog, response_cf_cn,response_pwp, alpha, merge_method)
%MERGERESPONSES interpolates the two responses with the hyperparameter ALPHA
    ...
    if strcmp(merge_method,'adaptive')
        
        response=0.4*response_cf_hog+0.3*response_cf_cn+0.3*response_pwp;
        %response=0.3*response_cf_hog+0.1*response_cf_cn+0.6*response_pwp;
    end
